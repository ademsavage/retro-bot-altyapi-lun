...
module.exports.run = async (bot, message, args, client) => {
	...
	var mutelirolu = await db.fetch(`Mrol_${message.guild.id}`);
	
	if(!mutelirolu) return message.channel.send(`**Mute Rolü** Ayarlanamamış! *${prefix}mute-rol @rol*`)
...
message.channel.send(
    ...
    .setFooter("© designed twenzy - RetroLUN", bot.user.avatarURL())
  );
  
  message.channel.send(`<@${mutekisi.id}> **başarıyla mutelendi!**`)
